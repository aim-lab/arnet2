...
def compute_features_notching(ecg, lead, fiducial_points, fs, factor=1000):
    """
    ...
    if len(indexes_notching) > 0:
        lengths = np.diff(indexes_notching) * units
        ...
        features_notching = pd.DataFrame({'NW1_' + str(lead): [lengths[0]],
                                          'NW2_' + str(lead): [lengths[1]],
                                          'ND1_' + str(lead): [depths[0]],
                                          'ND2_' + str(lead): [depths[1]],
                                          'Notch_Area_' + str(lead): [area]
                                          })
    else:
        features_notching = pd.DataFrame({'NW1_' + str(lead): [0],
                                          'NW2_' + str(lead): [0],
                                          'ND1_' + str(lead): [0],
                                          'ND2_' + str(lead): [0],
                                          'Notch_Area_' + str(lead): [0]
                                          })
...
def compute_features_slurring(ecg, lead, fiducial_points, fs):
    """
    ...
    if len(notching_indexes) > 0:
        ...
        if notching_indexes[2] - notching_indexes[1] > int(0.02 * 500):
            ...
            ratio4 = 0
            diff4 = 0
        features_slurring = pd.DataFrame({'MAX1_' + str(lead): [maximum1],
                                          'MAX2_' + str(lead): [maximum2],
                                          'Ratio1_' + str(lead): [ratio1],
                                          'Diff1_' + str(lead): [diff1],
                                          'Ratio2_' + str(lead): [ratio2],
                                          'Diff2_' + str(lead): [diff2],
                                          'Ratio3_' + str(lead): [ratio3],
                                          'Diff3_' + str(lead): [diff3],
                                          'Ratio4_' + str(lead): [ratio4],
                                          'Diff4_' + str(lead): [diff4]
                                          })
    else:
        features_slurring = pd.DataFrame({'MAX1_' + str(lead): [0],
                                          'MAX2_' + str(lead): [0],
                                          'Ratio1_' + str(lead): [0],
                                          'Diff1_' + str(lead): [0],
                                          'Ratio2_' + str(lead): [0],
...
def extraction_feature_wavedet_BBB(filename, ecg, freq, runtime, list_lead, wavedet_3D_dict, factor=1000, qrs_peak=[]):
    signal_DataFrame = pd.DataFrame()
    for i, lead in enumerate(list_lead):
        feat_lead = pd.DataFrame({'NW1_' + str(lead): [0],
                                  'NW2_' + str(lead): [0],
                                  'ND1_' + str(lead): [0],
                                  'ND2_' + str(lead): [0],
                                  'Notch_Area_' + str(lead): [0],
                                  ...
                                  'Ratio4_' + str(lead): [0],
                                  'Diff4_' + str(lead): [0],
                                  'Main_deflection_' + str(lead): [0]
                                  })
        wavedet_3D_dict_i = wavedet_3D_dict[i]
        qrs_points = np.asarray(wavedet_3D_dict_i['R'], dtype=np.double)
        # temp = qrs_points[np.abs(ecg[lead][qrs_points.astype(int)]) > np.abs(
        #     np.quantile(np.abs(ecg[lead][qrs_points.astype(int)]), .2))]
                ...
                if len(representative_qrs) > 0:
                    ...
                    if representative_dict['QRSon'].size > 0 and representative_dict['QRSoff'].size > 0:
                        ...
                        if representative_dict['QRSon'].size > 0 and representative_dict['QRSoff'].size > 0:
                            ...
                            feat_lead = extraction_feature_BBB(representative_qrs, lead, freq, representative_dict, factor)
...
def main_deflection_qrs(ecg, fiducial_points, lead):
    deflection = 0
    ...
    if off > on:
        ...
        if np.asarray(ecg)[main_pos + on] < 0:
            deflection = 1
    features = pd.DataFrame({'Main_deflection_' + str(lead): [deflection]})
    return features


def extraction_feature_BBB(representative_ecg, lead, freq,
                           template_dict, factor, preprocess=False):
    feat = pd.concat([compute_features_notching(representative_ecg, lead, template_dict, freq, factor),
                      compute_features_slurring(representative_ecg, lead, template_dict, freq)], axis=1)
    feat = pd.concat([feat, main_deflection_qrs(representative_ecg, template_dict, lead=lead)], axis=1)
    return feat
