...
from Preprocessing_features import compute_mean, compute_std, compute_median, minimum, maximum
...
def metrics(dRR):
    ...
    # insert your path
    bin_c = sio.loadmat(str(cts.MATLAB_TEST_VECTORS_DIR / 'edges_hist.mat'))['edges'][0][0][
...
def extraction_feature_AF(freq, dict_features, factor, lead):
    units = (factor / freq)  # ms
    R_points = dict_features['R']
    # R_points = dict_features[lead]
    RR_index = R_points[:-1] * R_points[1:] > 0
    rr = np.asarray(comp_diff(R_points))
    rr = rr[RR_index] * units
    ...
    feat = pd.DataFrame({'cosEn_' + str(lead): [comp_cosEn(rr)],
                         ...
                         'IrrEv_' + str(lead): [comp_IrrEv(rr)],
                         'minRR_' + str(lead): [comp_minRR(rr)],
                         'medHR_' + str(lead): [comp_medHR(rr/factor)],
                         'AVNN_' + str(lead): [comp_AVNN(rr)],
                         'SDNN_' + str(lead): [comp_SDNN(rr)],
                         'SEM_' + str(lead): [comp_SEM(rr)],
                         'RMSSD_' + str(lead): [comp_RMSSD(rr)],
                         'PNN20_' + str(lead): [comp_PNN20(rr/factor)],
                         'CV_' + str(lead): [comp_CV(rr)],
                         'PNN50_' + str(lead): [comp_PNN50(rr/factor)]
                         })
    return feat
