...
def load_reannotated(db, pat_list):
    ...
    rr, rrt, y, win_start, win_end, win_lab = db.return_rr(pat_list=pat_list,
                                                           return_binary=False)  # y is the true label per window
    if db.name == 'CPSCDB':
...
def add_diagnosis_col(df):
    db = RBAFDB_Parser(load_on_start=False)
    db.record_diagnosis()
    df_diagnosis = pd.DataFrame.from_dict(db.diagnosis_dict, orient='index', columns=['diagnosis'])
    df.diagnosis = df.holter_id.map(df_diagnosis.diagnosis)
    df.loc[df.diagnosis.str.contains("PerAF|PAF"), 'AF'] = 1
    df.loc[df.diagnosis.str.contains("AFL"), 'AFL?'] = 1
    return df
...
def plot_cosEn(cosEn1, cosEn2, test_dict, savefig=False, savedir=None, dpi=400):
    ...
    print(
        f"Female AF CosEn Quantile: {np.median(cosEn1[ids_AF_win_F]):.2f}, ({np.quantile(cosEn1[ids_AF_win_F], 0.25):.2f}-{np.quantile(cosEn1[ids_AF_win_F], 0.75):.2f}) ")
    print(
        f"Man AF CosEn Quantile: {np.median(cosEn2[ids_AF_win_M]):.2f}, ({np.quantile(cosEn2[ids_AF_win_M], 0.25):.2f}-{np.quantile(cosEn2[ids_AF_win_M], 0.75):.2f}) ")
...
if __name__ == '__main__':
    ...
    test_dict_XGB['low_age_group'], test_dict_XGB['mid_age_group'], test_dict_XGB[
        'high_age_group'] = data_loading.group_age(test_dict_XGB['test_all'])
    ...
    print(
        f'Number of AFL windows For male: {AFL_wins_M}, percentage out of all AFL windows: {np.round(100 * (AFL_wins_M / AFL_wins), 2)}')
    print(f'Prevalence of AFL in male: {np.round(100 * (AFL_wins_M / AF_wins_M), 2)}')

    print(
        f'Number of FN AFL windows in females: {len(FN_all.loc[FN_all.id.isin(ids_F) & FN_all.lab_rhythm.eq(cts.rhythms_dict["AFL"])])},'
    ...
    print(
        f'Number of FN AFIB windows in females: {len(FN_all.loc[FN_all.id.isin(ids_F) & FN_all.lab_rhythm.eq(cts.rhythms_dict["AFIB"])])},'
    ...
    print(
        f'Number of FN AFL windows in male: {len(FN_all.loc[FN_all.id.isin(ids_M) & FN_all.lab_rhythm.eq(cts.rhythms_dict["AFL"])])},'
    ...
    print(
        f'Number of FN AFIB windows in male: {len(FN_all.loc[FN_all.id.isin(ids_M) & FN_all.lab_rhythm.eq(cts.rhythms_dict["AFIB"])])},'
    ...
    print(
        f'Number of AFL windows For age group 60 to 75: {AFL_wins_mid}, percentage out of all AFL windows: {np.round(100 * (AFL_wins_mid / AFL_wins), 2)}')
    print(
        f'Number of AFL windows For age group 75 and above: {AFL_wins_high}, percentage out of all AFL windows: {np.round(100 * (AFL_wins_high / AFL_wins), 2)}')

    print(
        f'#AFL windows For age group le 60: {AFL_wins_low} vs #AF windows {AF_wins_low}, %AFL out of AF windows: {np.round(100 * (AFL_wins_low / LAB_wins_low), 2)},%AF out of AF windows: {np.round(100 * (AF_wins_low / LAB_wins_low), 2)}')
    print(
        f'#AFL windows For age group 60 to 75: {AFL_wins_mid} vs #AF windows {AF_wins_mid}, %AFL out of AF windows: {np.round(100 * (AFL_wins_mid / LAB_wins_mid), 2)},%AF out of AF windows: {np.round(100 * (AF_wins_mid / LAB_wins_mid), 2)}')
    print(
        f'#AFL windows For age group 75 and above: {AFL_wins_high} vs #AF windows {AF_wins_high}, %AFL out of AF windows: {np.round(100 * (AFL_wins_high / LAB_wins_high), 2)},%AF out of AF windows: {np.round(100 * (AF_wins_high / LAB_wins_high), 2)}')
