...
if __name__ == '__main__':
    ...
    eclf = CostumedVotingClassifier(estimators=[(key, model['classifier']) for key, model in members.items()], weights=[1, 2, 1.5])
    ...
    for set_name, set_data in test_dict.items():
        ...
        mean_abs_afb_error_dict[set_name] = metrics.mean_abs_afb_error(X, y, yhat)  # takes a lot of time, so maybe we should comment it for now before making it faster
        ...
        if error_analysis:
            ...
            df = df.join(pd.DataFrame.from_dict(dict(zip([member + ' decision threshold' for member in members.keys()],
                                                    np.tile([model['best_th'] for model in members.values()], np.shape(y.reshape(-1, 1))).T)), orient='columns'))
            df['yhat'] = yhat
            df['lab'] = y
            df['start_time'] = t_s[:, 0]
            df['end_time'] = t_s[:, -1]
