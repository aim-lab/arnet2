...
def read_csv_with_headers(path, mask_pat=None):
    ...
    :return column: Dictionary containing as key the headers and as values the different corresponding columns.
    """
    f = open(path, 'r')
    reader = csv.reader(f)
...
# Calls C code for epltd dtection. Assumes the ecg file with .dat format is present in the code directory (parsing
# project). Assumes ID is received as a string with the correct format.
def epltd0_detector(id, prog_dir=cts.EPLTD_PROG_DIR.as_posix(), ecg_dir=cts.PARSING_PROJECT_DIR.as_posix()):
    """
     This function generates a wfdb annotation based on the 'epltd' detector containing the peaks of an input wfdb
    ECG recording.
    The function calls the C code for epltd detection previously compiled. By default, the function assumes the ECG
    recording is present in the
    directory under the name "id.dat, id.hea", and further assumes the signal has been sampled with a frequency of
    200 [Hz].
...
def wqrs_detector(id, prog_dir=cts.WQRS_PROG_DIR.as_posix(), ecg_dir=cts.PARSING_PROJECT_DIR.as_posix()):
    """
    This function generates a wfdb annotation based on the 'wqrs' detector containing the peaks of an input wfdb
    ECG recording.
    The function calls the C code for wqrs detection previously compiled. By default, the function assumes the ECG
    recording is present in the
    directory under the name "id.dat, id.hea", and further assumes the signal has been sampled with a frequency of
    200 [Hz].
    :param id: The ID of the patient.
...
def xqrs_detector(id, ecg_dir=cts.PARSING_PROJECT_DIR, n_windows=2000, pool=None):
    """
    This function generates a wfdb annotation based on the 'xqrs' detector containing the peaks of an input wfdb
    ECG recording.
    The function calls the xqrs_detect function from the wfdb.preprocessing library.
    By default, the function assumes the ECG recording is present in the
    directory under the name "id.dat, id.hea", and further assumes the signal has been sampled with a frequency of
    200 [Hz].
    :param id: The ID of the patient.
    :param ecg_dir: The directory of the wfdb ECG input recording.
    :param n_windows: The number of windows to divide the ECG into. Each window is duplicated to ensure all the peaks
     are detected.
    :param pool: Pool of multiprocessors to be used to divide the workload on several cores. Each core deals
     with a window at a time.
    """
    ...
    print('done')
    wfdb.wrann(id, 'xqrs', ann, symbol=['q'] * len(ann), write_dir=ecg_dir)


def gqrs_detector(id, ecg_dir=cts.PARSING_PROJECT_DIR, n_windows=2000, pool=None):
    """
    This function generates a wfdb annotation based on the 'gqrs' detector containing the peaks of an input wfdb ECG
    recording.
    The function calls the gqrs_detect function from the wfdb.preprocessing library.
    By default, the function assumes the ECG recording is present in the
    directory under the name "id.dat, id.hea", and further assumes the signal has been sampled with a frequency of
    200 [Hz].
    :param id: The ID of the patient.
    ...
    :param n_windows: The number of windows to divide the ECG into. Each window is duplicated to ensure all the peaks
     are detected.
    :param pool: Pool of multiprocessors to be used to divide the workload on several cores. Each core deals with a
    window at a time.
    """
    record = wfdb.rdrecord(str(ecg_dir / id))
    fs = record.fs
    ...
    wfdb.wrann(id, 'gqrs', ann, symbol=['q'] * len(ann), write_dir=ecg_dir)


def jqrs_detector(id, ecg_dir=cts.PARSING_PROJECT_DIR, n_windows=1, pool=None, fs=200, thresh=cts.SQI_WINDOW_THRESHOLD,
                  ref_period=0.52, debug=False):
    """
    This function generates a wfdb annotation based on the 'jqrs' detector containing the peaks of an input wfdb
    ECG recording.
    The function calls the function for jqrs detection impleneted above. By default, the function assumes the ECG
    recording is present in the
    directory under the name "id.dat, id.hea", and further assumes the signal has been sampled with a frequency of
    200 [Hz].
    ...
    :param n_windows: The number of windows to divide the ECG into. Each window is duplicated to ensure all the peaks
     are detected.
    :param pool: Pool of multiprocessors to be used to divide the workload on several cores. Each core deals with
     a window at a time.
    :param fs: sampling frequency (Hz)
    :param thresh: threshold (nu)
    :param ref_period: refractory period (sec)
    :param debug: plot results (boolean)
    """
    record = wfdb.rdrecord(str(ecg_dir / id))
    ecg = record.p_signal[:, 0]
    ...
    if n_windows > 1:
        ...
        ecg_wins = [np.tile(e, 2) for e in ecg_wins]
        fss = fs * np.ones(n_windows)
        thr = thresh * np.ones(n_windows)
        rp = ref_period * np.ones(n_windows)
        debug_win = debug * np.ones(n_windows)
        res = pool.starmap(jqrs, zip(ecg_wins, fss, thr, rp, debug_win))
        res = [res[i][res[i] > lengths[i]] - lengths[i] for i in range(len(res))]
        ann = np.concatenate(tuple([res[i] + borders[i] for i in range(n_windows)])).astype(int)
        if pool_to_close:
            pool.close()
    else:
        ann = jqrs(ecg, fs, thresh, ref_period, debug)
    wfdb.wrann(id, 'jqrs', ann, symbol=['q'] * len(ann), write_dir=ecg_dir)


def qrs_adjust(ecg, qrs, fs, inputsign, tol=0.05, debug=0):
    """
    This function is used to adjust the qrs location by looking for a local min or max around the input qrs points.
    ...
    if debug:
        plt.plot(ecg)
        plt.scatter(qrs, ecg[qrs], marker='x', color='r', label='qrs')
        plt.scatter(cqrs, ecg[cqrs], marker='x', color='purple', label='cqrs')
        plt.show()
        plt.close()
    return cqrs


def qrs_adjust_detector(ecg, qrs, fs, inputsign, tol=0.05, debug=0, n_windows=1, pool=None):
    """
    This function adjust the qrs location based on the 'qrs_adjust'.
    The function calls the 'qrs adjust' implemented above.
    By default, the function assumes the signal has been sampled with a frequency of 200 [Hz].
    :param ecg:         array of ecg signal amplitude (mV)
    :param qrs:         peak position (sample number)
    :param fs:          sampling frequency (Hz)
    :param inputsign:   sign of the peak to look for (-1 for negative and 1 for positive)
    ...
    if n_windows > 1:
        if pool is None:
            ...
            pool_to_close = True
        borders = np.round(np.linspace(0, len(ecg), n_windows + 1)).astype(int)
        borders = borders[np.where(borders <= qrs[-1])]
        ecg_wins = [ecg[borders[i]:borders[i + 1]] for i in range(len(borders) - 1)]
        qrs_wins = [qrs[np.where(np.logical_and(qrs >= borders[i], qrs < borders[i + 1]))] - borders[i] for i in
                    range(len(borders) - 1)]
        lengths = np.array([len(e) for e in ecg_wins])
        n_windows = len(lengths)
        ecg_wins = [np.tile(e, 2) for e in ecg_wins]
        qrs_wins = [np.tile(e, 2) for e in qrs_wins]
        fss = fs * np.ones(n_windows)
        debug_wins = debug * np.ones(n_windows)
        tol_wins = tol * np.ones(n_windows)
        inputsign_wins = inputsign * np.ones(n_windows)
        res = pool.starmap(qrs_adjust, zip(ecg_wins, qrs_wins, fss, inputsign_wins, tol_wins, debug_wins))
        # res = [res[i][res[i] > lengths[i]] - lengths[i] for i in range(len(res))]
        combined = list(itertools.chain.from_iterable([res[i] + borders[i] for i in range(n_windows)]))
        ann = np.array(combined).astype(int)
        if pool_to_close:
            pool.close()
    else:
